...
in
  stdenv.mkDerivation {
    ...
    installPhase = ''
      ...
      ${python.interpreter} -c "import sys, pip._internal as pip; sys.exit(pip.main(['install', '--force-reinstall', '--upgrade', 'pip', 'setuptools', '--no-index', '--find-links=file://$PWD/../index', '-v', '--target', '$out/base']))"
      PYTHONPATH=$out/base ${python.interpreter} -c "import sys, pip._internal as pip; sys.exit(pip.main(['install', '--force-reinstall', '--upgrade', 'wheel', '--no-index', '--find-links=file://$PWD/../index', '-v', '--target', '$out/extra']))"
      PYTHONPATH=$out/base ${python.interpreter} -c "import sys, pip._internal as pip; sys.exit(pip.main(['install', '--force-reinstall', '--upgrade', 'zc.buildout', 'zc.recipe.egg', 'buildout.requirements', '--no-index', '--find-links=file://$PWD/../index', '-v', '--target', '$out/extra']))"

      touch $out/extra/buildout/__init__.py
      touch $out/extra/zc/__init__.py
      touch $out/extra/zc/recipe/__init__.py

      echo -e "#!${python.interpreter}\nimport sys, pip._internal as pip; sys.exit(pip.main())" > $out/bin/pip
